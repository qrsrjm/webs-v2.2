...
#include "dsp/lrsunDsp.h"
//#include "dsp/dspArchive.h"
//#include "dsp/repDsp.h"
...
static void formDelArchive(webs_t wp, char_t *path, char_t *query);
static void formAlpuc(webs_t wp, char_t *path, char_t *query);
 ...
 */
    ...
    websFormDefine(T("formDelArchive"), formDelArchive);
    
    websFormDefine(T("formAlpuc"), formAlpuc);
...
{
    uint8_t Ch,En,no,Type;
    uint16_t Fc;
	fp32  Q,Gain;
    Ch=En=no=Type = 0;
    Q = Gain = 0.0;
    
	Q =    atof(websGetVar(wp, T("Q"), T("0")));	
	Gain = atof(websGetVar(wp, T("Gain"), T("0"))); 	
	Fc =   atoi(websGetVar(wp, T("Fc"), T("0"))); 
	Type=  atoi(websGetVar(wp, T("Type"), T("0")));	  
	En =   atoi(websGetVar(wp, T("En"), T("0")));	
	no =   atoi(websGetVar(wp, T("no"), T("0")));
	Ch=	   atoi(websGetVar(wp, T("Ch"), T("0")));
...
{ 
    ...
    setMFlag(0);
    int ret = deleteArchive(fileName);
    if (ret == -7) {
        printf("can't delete default archive(%s).\n",fileName);
    } else if (ret == -1) {
